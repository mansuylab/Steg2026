...
dea.RNA.Sperm <- function(pheno, strain, generation, sex, exclude = NULL, 
  ...
  transcripts_up <- rownames(rowData(se)$dea[rowData(se)$dea$PValue < 0.005 & rowData(se)$dea$logFC > 1,])
  transcripts_up_id <- gsub("\\..*", "",transcripts_up)
    
  transcripts_down <- rownames(rowData(se)$dea[rowData(se)$dea$PValue < 0.005 & rowData(se)$dea$logFC < -1,])
