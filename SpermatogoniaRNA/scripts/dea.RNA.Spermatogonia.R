# Function to run differential analysis on RNA-seq data

dea.RNA.Spermatogonia <- function(pheno, strain, generation, sex, exclude = NULL, 
  ...
  ## Extract transcript IDs
    transcripts_up <- rownames(rowData(se)$dea[rowData(se)$dea$padj < 0.05 & rowData(se)$dea$logFC > 1,])
  transcripts_up_id <- gsub("\\..*", "",transcripts_up)
    
  transcripts_down <- rownames(rowData(se)$dea[rowData(se)$dea$padj < 0.05 & rowData(se)$dea$logFC < -1,])
